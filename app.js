...
const getSetQuestion = question =>
  new Promise(res => rl.question(question, res));

const getSetDefaultQuestion = (question, defaultValue) =>
  new Promise(res => {
    rl.question(question, newValue =>
      res(newValue.length ? newValue : defaultValue)
    );
  });
...
const getVersions = () => {
  ...
  const increasedVersion = `${major}.${minor}.${parseInt(patch) + 1}`;
  return { curr: currVersion, increased: increasedVersion };
};
...
const init = async () => {
  try {
    ...
    const pushedAndCommited = await getSetDefaultQuestion(
      `Have you committed and pushed all files? (yes): `,
      "yes"
    );
    ...
    // ask for branch name
    const branchName = await getSetDefaultQuestion(
      `Branch name: ${currentBranch} (yes): `,
      currentBranch
    );
    const commitMessage = await getSetDefaultQuestion(
      "Commit name: new release (yes): ",
      "new release"
    );
    bundlePath = await getSetDefaultQuestion(
      `Bundle path ${bundlePath} (yes): `,
      bundlePath
    );
